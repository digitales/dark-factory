...
**Finding `ACF-01` (P0 strategic, L).** Migrate away from ACF Pro. This is a multi-phase engagement laid out in [08 — Roadmap](/reports/theme-only-audit-2/08-roadmap). Once complete, ACF Pro, `stoutlogic/acf-builder`, the `wpml_found_strings_in_block` compat layer in `WPML.php`, the excerpt compat layer in `RankMath.php`, and significant amounts of copy-paste code in every block disappear.
...
4. **Output is not escaped**. SVG content is interpolated directly into HTML. If the SVG file on disk were ever malicious (via arbitrary file upload; see [04](/reports/theme-only-audit-2/04-security#svg-uploads)), the JavaScript inside it executes with page privileges.

### Native replacement

For icons, build a sprite at build time (the existing `svg-spritemap-webpack-plugin` setup does this — see [03](/reports/theme-only-audit-2/03-performance)) and reference by `<use>`:
...
**Finding `RANKMATH-01` (P1, L).** Replace Rank Math with a thin in-theme SEO module + core sitemap + Redirection plugin. See [05 — Dependencies](/reports/theme-only-audit-2/05-dependencies) for the catalogue and [08 — Roadmap](/reports/theme-only-audit-2/08-roadmap) for phasing.
...
- Swiper is **always enqueued on every admin page** by `Admin::enqueueAdminScripts()`. This is unnecessary — the admin editor only needs Swiper previews for a handful of block types. Covered in [03](/reports/theme-only-audit-2/03-performance).
...
Next: [03 — Performance](/reports/theme-only-audit-2/03-performance)
