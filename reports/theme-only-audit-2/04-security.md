...
All findings are tagged with severity based on exploitability and impact. P0 items should be addressed as quick wins before the migration roadmap in [08](/reports/theme-only-audit-2/08-roadmap).
...
- **WPCS flags this automatically** (see [01 PHP-02](/reports/theme-only-audit-2/01-conventions-and-structure#coding-standards-php)). Adopting WPCS will force the fix.
...
**Finding `SEC-ESCAPE-01` (P1, M).** Systematic pass over every `template.php` to escape every interpolated value. ~40 templates × 15 minutes each = ~10 engineer-days. The WPCS adoption in [01](/reports/theme-only-audit-2/01-conventions-and-structure) will identify all missing escapes.
...
**Finding `SEC-MENU-01` (P2, S).** Rewrite menu-walker extensions using `nav_menu_link_attributes` filter (native) instead of `str_replace` on the output. See [02](/reports/theme-only-audit-2/02-non-native-apis#guiding-principle-prefer-core-hooks-over-custom) for the "prefer core hooks" principle.
...
**Finding `SEC-RM-PARSE-01` (P2, S).** Replace the regex with `parse_blocks($post->post_content)` and walk the tree. Part of the Rank Math migration ([02 RANKMATH-01](/reports/theme-only-audit-2/02-non-native-apis#rank-math-dependencies)).

## `Render::partial` extract() scope bleed

Already covered in [02 RENDER-01](/reports/theme-only-audit-2/02-non-native-apis#renderpartial-bugs). Security implications:
...
Next: [05 — Dependencies](/reports/theme-only-audit-2/05-dependencies)
