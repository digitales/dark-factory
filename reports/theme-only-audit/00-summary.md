...
The audit is written to serve this direction. Every finding in [02 — Non-native APIs](/reports/theme-only-audit/02-non-native-apis) is paired with its native WordPress replacement. Every block in [06 — Blocks and Components](/reports/theme-only-audit/06-blocks-and-components) gets a "native equivalent" migration target. [07 — Reusability and plugin extraction](/reports/theme-only-audit/07-reusability-and-plugin-extraction) catalogues the plugins to extract, and [08 — Roadmap](/reports/theme-only-audit/08-roadmap) sequences the work into executable phases.
...
- **Initial migration cost.** ~8 months of single-developer calendar time to land the full migration for one representative site, including data migration and QA. See [08 — Roadmap](/reports/theme-only-audit/08-roadmap).
...
|---|---|---|---|---|---|
| 1 | SEC-SVG-01 | SVG uploads allow arbitrary JS injection (no sanitisation) | **P0** | S | [04](/reports/theme-only-audit/04-security#svg-uploads) |
| 2 | SEC-SITESCRIPTS-01 | Site-scripts option injects arbitrary HTML/JS without escape | **P0** | M | [04](/reports/theme-only-audit/04-security#site-scripts-injection-from-theme-options) |
| 3 | TPL-01 | `single.php` omits `get_header()` / `get_footer()` | **P0** | S | [01](/reports/theme-only-audit/01-conventions-and-structure#template-hierarchy) |
| 4 | RENDER-01 | `Render::partial` has 4 correctness bugs (call_user_method, cache args, extract scope bleed, dead hooks) | P1 | M | [02](/reports/theme-only-audit/02-non-native-apis#renderpartial-bugs) |
| 5 | SEC-ESCAPE-01 | Block templates don't escape interpolated values | P1 | M | [04](/reports/theme-only-audit/04-security#template-output-escaping) |
| 6 | PERF-THEME-JSON-01 | `theme.json` is suppressed on the front end via `remove_global_styles` | P1 | M | [03](/reports/theme-only-audit/03-performance#themejson-disabled-on-the-front-end) |
| 7 | PLUGIN-ACTIVE-01 | `is_plugin_active()` called from front-end contexts — latent fatal | P1 | S | [02](/reports/theme-only-audit/02-non-native-apis#is_plugin_active-on-the-front-end) |
| 8 | PERF-PRELOAD-01 | Homebrew preload/onload CSS pattern — FOUC risk, replaceable with native `wp_style_add_data(..., 'path', ...)` | P1 | S | [03](/reports/theme-only-audit/03-performance#the-preload--onload-css-pattern) |
| 9 | PERF-SPEC-01 | Hand-rolled speculation-rules tag prerenders every link including admin | P1 | S | [03](/reports/theme-only-audit/03-performance#speculation-rules) |
| 10 | ASSET-VER-01 | `filemtime()` called per enqueue — should consume the webpack manifest | P1 | S | [03](/reports/theme-only-audit/03-performance#asset-versioning-filemtime-vs-manifest) |

The first three (SEC-SVG-01, SEC-SITESCRIPTS-01, TPL-01) should ship to all live sites **before** any architectural work begins. They are the total scope of Phase 0 in [08 — Roadmap](/reports/theme-only-audit/08-roadmap).
...
- **Frontend developer / designer** — read 03 (performance), 06 (blocks specifically), and [07 §the hook-first contract](/reports/theme-only-audit/07-reusability-and-plugin-extraction#the-hook-first-contract).
...
| Document | Count |
|---|---:|
| [01 — Conventions and structure](/reports/theme-only-audit/01-conventions-and-structure) | 18 |
| [02 — Non-native APIs](/reports/theme-only-audit/02-non-native-apis) | 19 |
| [03 — Performance](/reports/theme-only-audit/03-performance) | 20 |
| [04 — Security](/reports/theme-only-audit/04-security) | 14 |
| [05 — Dependencies](/reports/theme-only-audit/05-dependencies) | 9 |
| [06 — Blocks and components](/reports/theme-only-audit/06-blocks-and-components) | 9 + per-block minors |
| [07 — Reusability and extraction](/reports/theme-only-audit/07-reusability-and-plugin-extraction) | 16 |
| **Total** | **~105 findings** |
...
End of summary. The document set starts at [01 — Conventions and structure](/reports/theme-only-audit/01-conventions-and-structure).
