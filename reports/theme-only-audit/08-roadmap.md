...
This document sequences every finding from documents [01–07](/reports/theme-only-audit/00-summary) into an executable plan. Each phase declares its goal, scope, dependencies on earlier phases, effort estimate, and exit criteria.
...
| Task | Ref | Effort |
|---|---|---|
| Fix SVG upload — add sanitizer or remove mime whitelist | [04 SEC-SVG-01](/reports/theme-only-audit/04-security#svg-uploads) | 0.5d |
| Fix site-scripts injection — replace free-form paste or gate + escape everything | [04 SEC-SITESCRIPTS-01](/reports/theme-only-audit/04-security#site-scripts-injection-from-theme-options) | 1.5d |
| Fix `single.php` missing `get_header/get_footer` | [01 TPL-01](/reports/theme-only-audit/01-conventions-and-structure#template-hierarchy) | 0.25d |
...
| Task | Ref | Effort |
|---|---|---|
| Remove redundant `spl_autoload_register` | [01 ARCH-01](/reports/theme-only-audit/01-conventions-and-structure#autoloading) | 0.1d |
| Replace `is_plugin_active()` with `class_exists` / `defined` | [01 ARCH-03](/reports/theme-only-audit/01-conventions-and-structure) | 0.1d |
| Fix `404.php` placeholder content | [01 TPL-02](/reports/theme-only-audit/01-conventions-and-structure#template-hierarchy) | 0.5d |
| Fix `<h1>` missing `</h1>` in search.php | [01 TPL-03](/reports/theme-only-audit/01-conventions-and-structure#template-hierarchy) | 0.1d |
| Decide: keep or delete `templates/default.php` | [01 TPL-04](/reports/theme-only-audit/01-conventions-and-structure#template-hierarchy) | 0.25d |
| `style.css` — add required header fields | [01 STYLE-01](/reports/theme-only-audit/01-conventions-and-structure#stylecss-and-themejson-contracts) | 0.25d |
| Unify identity across `style.css`, `composer.json`, `package.json`, webpack defaults | [01 ID-01](/reports/theme-only-audit/01-conventions-and-structure#naming-and-identity) | 0.5d |
| Fix PHPCS target to 8.1 | [01 PHP-01](/reports/theme-only-audit/01-conventions-and-structure#coding-standards-php) | 0.25d |
| Adopt WordPress-Extra PHPCS ruleset + baseline | [01 PHP-02](/reports/theme-only-audit/01-conventions-and-structure#coding-standards-php) | 2d (initial baseline creation + trivial-fix pass) |
| Rewrite `Render::partial()` — fix `wp_cache_set` args + `call_user_method` + `extract` | [02 RENDER-01](/reports/theme-only-audit/02-non-native-apis#renderpartial-bugs) | 1.5d |
| Rewrite `Helpers::generateLink()` with structured attribute building | [04 SEC-GENERATELINK-01](/reports/theme-only-audit/04-security#helpersgeneratelink-string-concatenation) | 0.5d |
| Fix `SVG::get()` — memoise, sanitise, validate path, use `href` | [02 SVG-01](/reports/theme-only-audit/02-non-native-apis#svgget-and-svgicon) | 0.5d |
| Template escaping pass over every `template.php` | [04 SEC-ESCAPE-01](/reports/theme-only-audit/04-security#template-output-escaping) | 5d (40 templates) |
| Add nonce verification to Ajax handler | [04 SEC-AJAX-01](/reports/theme-only-audit/04-security#ajax-without-nonce-verification) | 0.25d |
| Fix GTM ID validation + escaping | [04 SEC-GTM-01](/reports/theme-only-audit/04-security#gtm-injection) | 0.25d |
| Move CPT registration from constructors to `init` hook | [02 CPT-01](/reports/theme-only-audit/02-non-native-apis#jjgraingerposttypes) | 0.5d |
| Replace `jjgrainger/posttypes` with native `register_post_type` | [02 POSTTYPES-01](/reports/theme-only-audit/02-non-native-apis#jjgraingerposttypes) | 0.5d |
| Fix menu walker — use `nav_menu_link_attributes` instead of `str_replace` | [04 SEC-MENU-01](/reports/theme-only-audit/04-security#menu-walker-and-mega-menu-html-injection) | 0.5d |
| Fix share URL construction — `add_query_arg` + HTTPS | [04 SEC-SOCIAL-01](/reports/theme-only-audit/04-security#social-sharing-url-construction) | 0.25d |
| Remove dead `start_operation` / `end_operation` hooks | [02 OPS-01](/reports/theme-only-audit/02-non-native-apis#custom-ops-hooks) | 0.1d |
| Remove unused `jeroendesloovere/vcard` | [05 DEPS-VCARD-01](/reports/theme-only-audit/05-dependencies#composer-dependencies-theme) | 0.1d |
| Replace service-locator in `functions.php` with hooked bootstrap | [01 ARCH-02](/reports/theme-only-audit/01-conventions-and-structure#autoloading) | 1d |
...
| Task | Ref | Effort |
|---|---|---|
| Replace `filemtime()` with webpack-manifest-driven versioning | [03 ASSET-VER-01](/reports/theme-only-audit/03-performance#asset-versioning-filemtime-vs-manifest) | 1d |
| Standardise `wp_register_script` strategies | [03 ENQUEUE-01](/reports/theme-only-audit/03-performance#enqueue-strategies) | 0.5d |
| Declare explicit script dependencies per block | [03 ENQUEUE-02](/reports/theme-only-audit/03-performance#enqueue-strategies) | 0.5d |
| Remove preload/onload CSS pattern; replace with `wp_style_add_data(..., 'path', ...)` | [03 PERF-PRELOAD-01](/reports/theme-only-audit/03-performance#the-preload--onload-css-pattern) | 0.5d |
| Scope admin Swiper enqueue to block-editor screens | [03 PERF-SWIPER-ADMIN-01](/reports/theme-only-audit/03-performance#swiper-loading) | 0.1d |
| Delete hand-rolled speculation-rules tag | [03 PERF-SPEC-01](/reports/theme-only-audit/03-performance#speculation-rules) | 0.1d |
| `theme.json` decision — commit or delete | [03 PERF-THEME-JSON-01](/reports/theme-only-audit/03-performance#themejson-disabled-on-the-front-end) | 2d |
| Add `composer audit` + `pnpm audit` to CI | [05 DEPS-AUDIT-01](/reports/theme-only-audit/05-dependencies#upgrade-hygiene) | 0.25d |
| Cache `Cards::queryPosts` + clean up Person meta_query | [03 PERF-QUERY-01, 02](/reports/theme-only-audit/03-performance#database-queries) | 1d |
| Caps-added options — remove from autoload | [03 PERF-CAPS-02](/reports/theme-only-audit/03-performance#capability-writes-on-every-request) | 0.25d |
| Preload variable fonts + font-display: swap | [03 PERF-FONT-01](/reports/theme-only-audit/03-performance#font-loading) | 0.5d |
| Subset fonts to script(s) used | [03 PERF-FONT-02](/reports/theme-only-audit/03-performance#font-loading) | 0.25d |
...
**Goal**: Add the [proposed hook surface](/reports/theme-only-audit/07-reusability-and-plugin-extraction#proposed-hook-surface) to the current codebase. No behaviour changes — purely additive. Preconditions for Phase 7 (plugin extraction).
...
| Task | Ref | Effort |
|---|---|---|
| Write in-theme SEO module (meta tags, JSON-LD schema, robots, sitemap) | [02 RANKMATH-01](/reports/theme-only-audit/02-non-native-apis#rank-math-dependencies) | 3d |
...
| Consider Swiper → CSS scroll-snap or lazy-import per block | [02 SWIPER-02](/reports/theme-only-audit/02-non-native-apis#swiper) | 2d |
| Consider removing jQuery from FormBlock | [02 JQUERY-01](/reports/theme-only-audit/02-non-native-apis#jquery) | 0.25d |
...
**Goal**: Extract the reusable subsystems into standalone GitHub repos + Composer packages per [07](/reports/theme-only-audit/07-reusability-and-plugin-extraction).
...
|---|---|---|
| Run a Bedrock / Composer retrospective, capture findings in a 1-page doc | [07 DX-01](/reports/theme-only-audit/07-reusability-and-plugin-extraction#dx-01--bedrock--composer-retrospective-precondition) | 1d |
| Extend `setup/create.sh` to write Composer VCS entries + auth scaffolding | [07 DX-02](/reports/theme-only-audit/07-reusability-and-plugin-extraction#dx-02--starter-setup-script-writes-composer-vcs-entries) | 1d |
| Write `docs/COMPOSER-ONBOARDING.md` (first-time setup + top-10 errors) | [07 DX-03](/reports/theme-only-audit/07-reusability-and-plugin-extraction#dx-03--composer-onboarding-documentation) | 1d |
| Add Makefile shortcuts (`make edwp-add`, `edwp-update`, `edwp-status`) | [07 DX-04](/reports/theme-only-audit/07-reusability-and-plugin-extraction#dx-04--makefile-shortcuts) | 0.5d |
| Commit `.github/workflows/composer.yml` template | [07 DX-05](/reports/theme-only-audit/07-reusability-and-plugin-extraction#dx-05--client-repo-ci-workflow-template) | 1d |
| Budget for paired onboarding of the first developer per consuming project | [07 DX-06](/reports/theme-only-audit/07-reusability-and-plugin-extraction#dx-06--paired-onboarding-for-the-first-consuming-project) | 0.5d × per developer |
...
| `edwp/wp-theme-render` library | 2d | [07 REUSE-LIB-RENDER](/reports/theme-only-audit/07-reusability-and-plugin-extraction#candidate-library-catalogue) |
| `edwp/wp-svg-sprite` library | 2d | [07 REUSE-LIB-SVG](/reports/theme-only-audit/07-reusability-and-plugin-extraction#candidate-library-catalogue) |
| `edwp/wp-theme-utils` library | 1d | [07 REUSE-LIB-UTILS](/reports/theme-only-audit/07-reusability-and-plugin-extraction#candidate-library-catalogue) |
| `edwp-core` MU plugin (media hardening + SVG) | 3d | [07 REUSE-EXTRACT-CORE](/reports/theme-only-audit/07-reusability-and-plugin-extraction#candidate-plugin-catalogue) |
| `edwp-admin` plugin (login branding + widgets) | 2d | [07 REUSE-EXTRACT-ADMIN](/reports/theme-only-audit/07-reusability-and-plugin-extraction#candidate-plugin-catalogue) |
| `edwp-site-options` plugin | 5d | [07 REUSE-EXTRACT-SITE-OPTIONS](/reports/theme-only-audit/07-reusability-and-plugin-extraction#candidate-plugin-catalogue) |
| `edwp-modals` plugin | 5d | [07 REUSE-EXTRACT-MODALS](/reports/theme-only-audit/07-reusability-and-plugin-extraction#candidate-plugin-catalogue) |
| `edwp-people` plugin | 2d | [07 REUSE-EXTRACT-PEOPLE](/reports/theme-only-audit/07-reusability-and-plugin-extraction#candidate-plugin-catalogue) |
| `edwp-testimonials` plugin | 2d | [07 REUSE-EXTRACT-TESTIMONIALS](/reports/theme-only-audit/07-reusability-and-plugin-extraction#candidate-plugin-catalogue) |
| `edwp-gravityforms-styling` plugin | 1d | [07 REUSE-EXTRACT-GF](/reports/theme-only-audit/07-reusability-and-plugin-extraction#candidate-plugin-catalogue) |
| `edwp-classic-editor` plugin | 1.5d | [07 REUSE-EXTRACT-CLASSIC](/reports/theme-only-audit/07-reusability-and-plugin-extraction#candidate-plugin-catalogue) |
| Set up GitHub repos + Composer VCS config in starter | 1d | [07 DIST-01](/reports/theme-only-audit/07-reusability-and-plugin-extraction#distribution-strategy) |
| CI release automation per repo (version-from-tag) | 1d per repo × ~10 repos | [07 DIST-02](/reports/theme-only-audit/07-reusability-and-plugin-extraction#distribution-strategy) |
...
Next: [00 — Summary](/reports/theme-only-audit/00-summary)
