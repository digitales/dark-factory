...
- **Used by**: Every block + component + CPT + option lookup — see [02 ACF-01](/reports/theme-only-audit/02-non-native-apis#acf-pro-advanced-custom-fields) for the full coupling map.
...
- **Cost to remove**: Multi-week engagement. See [08 Phase 3–4](/reports/theme-only-audit/08-roadmap).
...
Covered in detail in [02 POSTTYPES-01](/reports/theme-only-audit/02-non-native-apis#jjgraingerposttypes).
...
- **Verdict**: **Keep optional**. See [02 SWIPER-02](/reports/theme-only-audit/02-non-native-apis#swiper) for lazy-import / CSS scroll-snap replacement strategies.
...
- **Note**: See [03 PERF-FONT-02](/reports/theme-only-audit/03-performance#font-loading) — subset to latin-only if applicable.
...
| `webpack-manifest-plugin` | Manifest generation | Keep (see [03 ASSET-VER-01](/reports/theme-only-audit/03-performance#asset-versioning-filemtime-vs-manifest)) |
...
For the native block migration (Phase 3 in [08](/reports/theme-only-audit/08-roadmap)), `@wordpress/scripts` is the canonical choice. For the existing non-block JS (`scripts.js`, `modal.js`, `lazyload.js`, `mobile-menu.js`, etc.), the current webpack works fine.
...
Next: [06 — Blocks and Components (exhaustive)](/reports/theme-only-audit/06-blocks-and-components)
