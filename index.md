---
layout: home
title: Dark Factories & AI-Generated Code
titleTemplate: Research Report
hero:
  name: Dark Factories & AI-Generated Code
  text: Research Report
  tagline: Spec-driven development, consultancy dynamics, and adoption for PHP, WordPress and Laravel teams.
  actions:
    - text: Read the report
      link: /report/
      type: primary
    - text: Elixirr Digital
      link: https://www.elixirrdigital.com
      type: secondary
features:
  - title: What is a Dark Factory?
    details: A development model where humans write specifications and approve outcomes; AI agents implement, validate, and ship. No human writes or reviews code.
    link: /report/#2-what-is-a-dark-factory
  - title: Consultancy environment
    details: How the workflow applies when the dev team sits inside a consultancy (e.g. Elixirr); roles of consultants, developers, and account managers.
    link: /report/#4a-dark-factory-in-a-consultancy-environment
  - title: Adopting the shift
    details: Workflow changes, Jira and GitHub, skills by level (junior to lead), constitutions, and agent rules—all in one place.
    link: /report/#8-adopting-the-shift-workflow-tools-skills-and-constitution
footer: Part of Elixirr Digital — Your digital partner. Experience digital solutions that challenge the ordinary.
---

<div class="vp-doc" style="margin-top: 2rem;">

**Document type:** Research report  
**Audience:** PHP-based development team (WordPress & Laravel)  
**Purpose:** Inform adoption of spec-driven and AI-assisted workflows  
**Date:** February 2026

The full report is structured so it can be read section by section from the sidebar or as a single document. The source Markdown is retained in the project at `docs/DARK_FACTORY_AI_CODE_RESEARCH_REPORT.md`.

</div>
